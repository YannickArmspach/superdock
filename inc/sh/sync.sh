...
case $WAY in
	...
	from)
		...
		docker-compose -f $SUPERDOCK_CORE_DIR/docker/config.yml exec webserver sh -c "mysql --host=superdock_database --user=root --password=root db < /var/www/html/superdock/database/local.sql"
	;;
	...
	to)
		...
		docker-compose -f local/docker/config.yml exec webserver sh -c "mysqldump --host=superdock_database --user=root --password=root db > /var/www/html/local/db/local.sql"
